...
data = pd.read_csv('0000',delimiter='\s+') # Read file of the Model
...
#Extracting only concerned part from data1
#data=pd.DataFrame(data[['r','Lx','Ly','Lz']])
...
ax.plot(meanr,ratiopx,label='Model Distribution - Lx')
#ax.plot(meanr,ratiopy, c = 'green',label='Model Distribution - Ly')
#ax.plot(meanr,ratiopz, c = 'purple', label='Model Distribution - Lz')
ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
plt.title("Radial Distribution of Angular Momenta at Time 2.00000000E+01")

ax2 = ax.twinx()
ax2.set_ylim(1, 0)
ax.set_ylim(0, 1)
ax.axhline(0.5,c='red',ls='--',alpha=.7, label='Ideal Distribution')

#defining plot axis 
ax.set_xlabel("Radius")
ax.set_ylabel("N(Lx+)/N")
ax2.set_ylabel("N(Lx-)/N")                                                                
ax.legend() 
plt.plot()
plt.show()
